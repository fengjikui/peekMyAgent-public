...
- macOS、Linux、WSL，或 Windows 上的 Git Bash/其他类 Unix shell 环境用于一行安装脚本。
- Node.js 18 或更高版本。
...
如果你的 Claude Code 不是通过 shell 环境变量配置，而是通过 Claude Code settings 文件配置，peekMyAgent 也会读取其中的 `ANTHROPIC_BASE_URL` 以及相关 Claude Code 环境值：

- `~/.claude/settings.json`
- `~/.claude/settings.local.json`
- `<你的项目目录>/.claude/settings.json`
...
显式 shell 环境变量优先级仍然最高。如果你只想为 peekMyAgent 强制指定上游地址，可以使用 `PEEK_CLAUDE_TARGET_BASE_URL`，不用改 Claude Code 自己的配置。
...
安装脚本会把仓库 clone 到 `~/.peekmyagent/app`，并在 `~/.local/bin/peekmyagent` 创建一个 CLI shim。如果 `~/.local/bin` 不在你的 `PATH` 中，安装脚本会打印需要添加的命令。
...
如果安装脚本提示 `~/.local/bin` 不在 `PATH` 中，把这一行加入你的 shell 配置文件，然后重启终端：
